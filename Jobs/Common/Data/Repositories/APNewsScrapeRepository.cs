...
public class APNewsScrapeRepository(NewsSnapshotDatabase database)
{
    public async Task CreateTableAsync()
    {
        string script = "CreateAPNewsScrapeTableV1.1";
    ...
    public async Task<long> CreateAsync(APNewsScrape job)
    {
        string? source = job.UseMainPageTestFile ? job.MainPageTestFile : job.SourceUri.AbsoluteUri;

        string commandText = @"
            INSERT INTO ap_news_scrape_job (source_name, source, job_started_on) 
        ...
        throw new InvalidOperationException("Insert into ap_news_scrape_job failed, no row id returned.");
    }

    public async Task UpdateAsync(APNewsScrape job)
    {
        ...
        if (rowsAffected == 0)
            throw new InvalidOperationException($"No record found with id = {job.Id} to update in the ap_news_scrape_job table.");
    }
