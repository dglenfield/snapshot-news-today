...
namespace NewsScraper.Utilities;
...
{
    private static readonly LogLevel _applicationLogLevel;
    private static readonly string _logDirectory;
    private static readonly bool _logToConsole;
    private static readonly bool _logToFile;

    /// <summary>
    /// Initializes static configuration settings for the Logger class, including log file paths, console and file
    /// logging options, and the default application log level.
    /// </summary>
    /// <remarks>This static constructor reads logging configuration from the application's settings file and
    /// user secrets. If file logging is enabled, the log directory is created if it does not already exist. Any
    /// exceptions encountered during initialization are logged and rethrown.</remarks>
    ...
    static Logger()
    {
        try
        {
            ...
            _logDirectory = config["Logging:LogToFile:FilePath"] ?? string.Empty;
            _logToConsole = bool.Parse(config["Logging:LogToConsole:Default"] ?? "false");
            _logToFile = bool.Parse(config["Logging:LogToFile:Default"] ?? "false");

            if (_logToFile && string.IsNullOrWhiteSpace(_logDirectory))
                throw new InvalidOperationException("Logging:FilePath:Default must be set when LogToFile is true.");
            
            if (_logToFile)
                Directory.CreateDirectory(_logDirectory);

            var loggingLevel = config["Logging:LogLevel:Default"] ?? "Information";
            _applicationLogLevel = loggingLevel.ToLower() switch
            {
    ...
    /// specify explicitly.</param>
    internal static void Log(string message, LogLevel level = LogLevel.Info, string? logFileName = null,
        ...
        [CallerLineNumber] int sourceLineNumber = 0)
    {
        ...
        string logEntry = $"{timestamp} [{level.ToString().ToUpper()}] \t{message} <{className}.{memberName}> (Line {sourceLineNumber})";
